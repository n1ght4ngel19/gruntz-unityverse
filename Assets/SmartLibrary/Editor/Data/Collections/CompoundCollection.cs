...
namespace Bewildered.SmartLibrary
{
    ...
    {
        [SerializeField] private List<CollectionReference<LibraryCollection>> _autoSourceCollections = new List<CollectionReference<LibraryCollection>>();
        [SerializeField] private List<CollectionReference<LibraryCollection>> _sourceCollections = new List<CollectionReference<LibraryCollection>>();
        [SerializeField] private SerializableDictionary<string, List<UniqueID>> _itemsSourcesMap = new SerializableDictionary<string, List<UniqueID>>();
        ...
        {
            // The collection can be added through the inspector so we make sure to remove it in case that has happened.
            _sourceCollections.Remove(this);

            PrepareItemChange();
            
            _autoSourceCollections.Clear();
            foreach (LibraryCollection subcollection in Subcollections)
            {
            ...
            {
                // The collection may not have been set in the inspector yet.
        ...
        private bool AddSourcedItem(LibraryItem item, LibraryCollection source)
        {
            ...
            if (_itemsSourcesMap.TryGetValue(item.GUID, out List<UniqueID> sources))
            {
        ...
        private void UpdateGroupOption(AutoSourceOption autoGrouping)
        {
            ...
            if (autoGrouping == AutoSourceOption.TopSubcollectionsOnly)
            {
                if (previousAutoGroup == AutoSourceOption.AllSubcollections)
                {
                    ...
                    for (int i = _autoSourceCollections.Count - 1; i >= 0; i--)
                    {
                        if (_autoSourceCollections[i].Collection.Parent != this)
        ...
        private void ClearAutoSources()
        {
            ...
            for (int i = _autoSourceCollections.Count - 1; i >= 0; i--)
            {
                removedItems.AddRange(RemoveSourceItems(_autoSourceCollections[i].Collection, _autoSourceCollections[i], false));
                _autoSourceCollections.RemoveAt(i);
            }
        ...
        private void OnLibraryHierarchyChanged(LibraryHierarchyChangedEventArgs args)
        {
            ...
            // If the changed collection is in the autoSource list but is no longer a descendant,
            // we need to remove it from the list since only descendants can be in the list.
            if (_autoSourceCollections.Contains(args.subcollection))
            {
                ...
                {
                    // There are 5 undo increments between the start of a hierarchy remove change and the end.
                    // So we need to go back 5 to properly group the operations.
                    currentGroup -= 5;
        ...
        {
            for (int i = 0; i < _sourceCollections.Count; i++)
            {
                _sourceCollections[i] = _sourceCollections[i].Resolve();
            }
            
            for (int i = 0; i < _autoSourceCollections.Count; i++)
            {
                _autoSourceCollections[i] = _autoSourceCollections[i].Resolve();
            }
        }
    }
}