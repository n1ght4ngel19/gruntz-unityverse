...
namespace Bewildered.SmartLibrary.UI
{
    internal class ReorderableList : VisualElement
    {
        ...
        public bool IsCountFieldEditable
        {
            get
            {
                if (_countField != null)
                    return _countField.enabledSelf;
                return false;
            }
            set
            {
                if (_countField != null)
                    _countField.SetEnabled(value);
            }
        }
        ...
        private void Initialize()
        {
            ...
            styleSheets.Add(LibraryUtility.LoadStyleSheet("ReorderableList"));
            styleSheets.Add(LibraryUtility.LoadStyleSheet($"ReorderableList{(EditorGUIUtility.isProSkin ? "dark" : "Light")}"));
            ...
            ScrollView.AddToClassList(itemContainerUssClassName);
            ...
            if (IsCollapsible && ListProperty != null)
            {
                _contentContainer.SetDisplay(ListProperty.isExpanded);
            }
        }
        ...
        private void CreateHeader()
        {
            if (_headerContainer != null)
                hierarchy.Remove(_headerContainer);
            
            if (IsCollapsible)
            {
                var foldout = new Foldout();
                foldout.AddToClassList(foldoutHeaderUssClassName);
                foldout.name = "headerLabel";

                foldout.RegisterValueChangedCallback(evt =>
                {
                    _contentContainer.SetDisplay(evt.newValue);
                    if (ListProperty != null)
                        ListProperty.isExpanded = evt.newValue;
                });

                if (ListProperty != null)
                    foldout.value = ListProperty.isExpanded;

                _headerContainer = foldout;

                if (ListProperty != null)
                {
                    _countField = new IntegerField();
                    _countField.BindProperty(ListProperty.FindPropertyRelative("Array.size"));
                    foldout.hierarchy[0].Add(_countField);
            ...
            else
            {
                ...
                _headerContainer.AddToClassList(headerUssClassName);
            }
            _headerContainer.name = "header";
            
            hierarchy.Insert(0, _headerContainer);

            if (ListProperty != null)
            {
                if (IsCollapsible)
                    ((Foldout)_headerContainer).text = ListProperty.displayName;
                else
