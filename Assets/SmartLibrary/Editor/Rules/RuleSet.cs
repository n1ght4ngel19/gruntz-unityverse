    ...
    public class RuleSet : IEnumerable<LibraryRuleBase>, IReadOnlyList<LibraryRuleBase>
    {
        ...
        public string GetSearchQuery()
        {
            ...
            foreach (var rule in _rules)
            {
                ...
                bool isRuleValid = rule.IsRuleValid();
                if (isRuleValid)
                {
                    if (!string.IsNullOrEmpty(fullSearchQuery))
                    {
                ...
                if (isRuleValid)
                    fullSearchQuery += rule.SearchQuery;
        ...
        public bool Evaluate(LibraryItem item)
        {
            ...
            foreach (var rule in _rules)
            {
                ...
                else // It will be false if the rule is invalid. It is easier to do this than to skip the code if the rule is invalid.
                    results.Push(false); 
