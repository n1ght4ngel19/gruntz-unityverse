...
internal class AnimationGeneratorWindow : EditorWindow {
	...
	private void OnEnable() {
		_internalPath = EditorPrefs.HasKey("AnimationGeneratorSpriteRendererInternalPath")
			? EditorPrefs.GetString("AnimationGeneratorSpriteRendererInternalPath")
			: "";

		_selectFromProjectView = EditorPrefs.HasKey("AnimationGeneratorSelectFromProjectView")
			? EditorPrefs.GetBool("AnimationGeneratorSelectFromProjectView")
			: false;
	...
	private void OnAddCallback(ReorderableList list, SerializedProperty animations) {
		...
		newAnim.FindPropertyRelative("_name").stringValue = MakeNameUnique(animations, _spritesheet.name, index);
		newAnim.FindPropertyRelative("_length").intValue = _sprites.Count;
	...
	private void DrawElementCallback(Rect rect, int index, SerializedProperty animations) {
		...
		autoFramerate.boolValue = EditorGUI.Toggle(
			rect,
			new GUIContent("AutoFramerate", "AutoFramerate will set the framerate equals to the number of frames so the animation duration will be 1 second"),
			autoFramerate.boolValue
		);

		if (autoFramerate.boolValue) {
			EditorGUI.BeginDisabledGroup(true);
			frameRate.intValue = 12;
		}
	...
	private void Update() {
		...
		if (_selectFromProjectView
			&& Selection.activeObject
			&& (Selection.activeObject as Texture2D) != null
	...
	private void OnGUI() {
		...
		EditorGUILayout.LabelField(
			new GUIContent(
				...
				"Select a Texture2D spritesheet asset with its Sprite Mode set to Multiple, ensure to properly cut the spritesheet into the Sprite Editor. This tool only support ordered sprites packing."
			)
		);
		...
		_selectFromProjectView = EditorGUILayout.ToggleLeft(
			new GUIContent("Switch on project view selection", "If enabled, selecting a Texture2D asset into the project view will open it on this window."),
			_selectFromProjectView
		);
		...
		if (_list.count > 0 && _spritesheet != null) {
			...
			int spriteEndIndex = spriteStartIndex + animations.GetArrayElementAtIndex(_list.index).FindPropertyRelative("_length").intValue - 1 < _sprites.Count
				? spriteStartIndex + animations.GetArrayElementAtIndex(_list.index).FindPropertyRelative("_length").intValue - 1
				: _sprites.Count - 1;
			...
			if (GUILayout.Button(
					new GUIContent(
						...
						canGenerateAll ? "Generate all animations on the list" : "Some animations on the list exceed the spritesheet sprites count."
					)
				)) {
			...
			if (GUILayout.Button(
					new GUIContent(
						...
						canGenerateOne ? "Generate selected animation on the list" : "The selected animation exceeds the spritesheet sprites count."
					)
				)) {
	...
	private void ResizeScrollView() {
		_currentScrollViewHeight = Mathf.Clamp(
			_currentScrollViewHeight,
			Mathf.Min(_list.GetHeight() + 100f - 16f, 200f),
			Mathf.Min(_list.GetHeight() + 100f - 16f, position.height - 200f)
		);
		...
		if (position.height > 400) {
			...
			if (_resize) {
				_currentScrollViewHeight = Mathf.Clamp(
					Event.current.mousePosition.y - 26f,
					Mathf.Min(_list.GetHeight() + 100f - 16f, 200f),
					Mathf.Min(_list.GetHeight() + 100f - 16f, position.height - 200f)
				);
			}
	...
	private void WriteFile(SerializedProperty animations, int index) {
		...
		string filePath = EditorUtility.SaveFilePanelInProject(
			"Choose where to save your AnimationClip assets",
			animInfo.FindPropertyRelative("_name").stringValue,
			"anim",
			"Save this animation"
		);
	...
	private void WriteFiles(SerializedProperty animations) {
		string folderPath = EditorUtility.OpenFolderPanel(
			"Choose a folder to create your AnimationClip assets",
			"Assets/Animationz/NewAnimz",
			""
		);
	...
	private static AnimationClip CreateAnimationClip(Sprite[] sprites, SerializedProperty animInfo, string path) {
		...
		AnimationClipSettings clipSetting = new AnimationClipSettings {
			...
			stopTime = (float)animInfo.FindPropertyRelative("_length").intValue / (float)animInfo.FindPropertyRelative("_frameRate").intValue,
		};
